...
async def menu_del_chat(callback: CallbackQuery, state: FSMContext):
    await callback.message.delete()
    await callback.message.answer(text=LEXICON['del_chat_menu'], reply_markup=kb.kb_delete_one_chat(bot_db.get_chats()))
